...
class MiIOService:
    ...
    async def miot_spec(self, type=None, format=None):
        ...
        if format == 'lite':
            return {s['description']: {'iid': s['iid']} | {(p['description'] + '=') if 'write' in p['access'] else p['description']: p['iid'] if 'read' in p['access'] else -p['iid'] for p in s.get('properties', [])} | {'@' + a['description']: a['iid'] for a in s.get('actions', [])} for s in services}
        ...
        result = '# Generated by https://github.com/Yonsm/MiService\n# ' + url + '\n\n' + STR_HEAD
        for s in services:
